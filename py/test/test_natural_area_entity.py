...
class TestNaturalAreaEntity:

    def test_should_create_instance(self):
        testsdk = ColombiaPublicSDK.test(None, None)
        ent = testsdk.NaturalArea(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.NaturalArea(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.NaturalArea(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _natural_area_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "natural_area." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set COLOMBIA_PUBLIC_TEST_NATURAL_AREA_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        natural_area_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.natural_area")))
        natural_area_ref01_data = None
        if len(natural_area_ref01_data_raw) > 0:
        ...
        natural_area_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(natural_area_ref01_data_dt0_loaded))
...
def _natural_area_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/natural_area/NaturalAreaTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "COLOMBIA_PUBLIC_TEST_NATURAL_AREA_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("COLOMBIA_PUBLIC_TEST_NATURAL_AREA_ENTID"))
