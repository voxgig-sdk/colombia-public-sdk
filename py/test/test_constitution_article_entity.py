...
class TestConstitutionArticleEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.ConstitutionArticle(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.ConstitutionArticle(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set COLOMBIA_PUBLIC_TEST_CONSTITUTION_ARTICLE_ENTID JSON to run live")
        client = setup["client"]
        ...
        constitution_article_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(constitution_article_ref01_data_dt0_loaded))
...
def _constitution_article_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("COLOMBIA_PUBLIC_TEST_CONSTITUTION_ARTICLE_ENTID"))
