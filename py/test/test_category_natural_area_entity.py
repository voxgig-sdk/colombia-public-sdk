...
class TestCategoryNaturalAreaEntity:

    def test_should_create_instance(self):
        testsdk = ColombiaPublicSDK.test(None, None)
        ent = testsdk.CategoryNaturalArea(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.CategoryNaturalArea(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.CategoryNaturalArea(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _category_natural_area_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "category_natural_area." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set COLOMBIA_PUBLIC_TEST_CATEGORY_NATURAL_AREA_ENTID JSON to run live")
        client = setup["client"]
        ...
        category_natural_area_ref01_data = None
        if len(category_natural_area_ref01_data_raw) > 0:
...
def _category_natural_area_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/category_natural_area/CategoryNaturalAreaTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("COLOMBIA_PUBLIC_TEST_CATEGORY_NATURAL_AREA_ENTID"))
