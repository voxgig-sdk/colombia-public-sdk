# ColombiaPublic SDK test runner
...
class ColombiaPublicTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ColombiaPublicTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ColombiaPublicTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ColombiaPublicTestRunner.getenv("COLOMBIA_PUBLIC_TEST_LIVE")
        override = ColombiaPublicTestRunner.getenv("COLOMBIA_PUBLIC_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ColombiaPublicTestRunner.getenv(key)
        ...
        explain = ColombiaPublicTestRunner.getenv("COLOMBIA_PUBLIC_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["COLOMBIA_PUBLIC_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ColombiaPublicTestRunner._test_control is not None:
            return ColombiaPublicTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ColombiaPublicTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ColombiaPublicTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ColombiaPublicTestRunner.load_test_control()
...
def load_env_local():
    ColombiaPublicTestRunner.load_env_local()


def env_override(m):
    return ColombiaPublicTestRunner.env_override(m)


def entity_data(v):
    return ColombiaPublicTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ColombiaPublicTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ColombiaPublicTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ColombiaPublicTestRunner.load_test_control()


def live_client_options():
    return ColombiaPublicTestRunner.live_client_options()


def live_delay_ms():
    return ColombiaPublicTestRunner.live_delay_ms()
