...
class TestTypicalDishEntity:

    def test_should_create_instance(self):
        testsdk = ColombiaPublicSDK.test(None, None)
        ent = testsdk.TypicalDish(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.TypicalDish(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.TypicalDish(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _typical_dish_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "typical_dish." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set COLOMBIA_PUBLIC_TEST_TYPICAL_DISH_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        typical_dish_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.typical_dish")))
        typical_dish_ref01_data = None
        if len(typical_dish_ref01_data_raw) > 0:
        ...
        typical_dish_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(typical_dish_ref01_data_dt0_loaded))
...
def _typical_dish_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/typical_dish/TypicalDishTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("COLOMBIA_PUBLIC_TEST_TYPICAL_DISH_ENTID"))
