...
class NativeCommunityEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return NativeCommunityEntity(self._client, opts)
    ...
    def data_get(self) -> NativeCommunity:
    ...
    def match_get(self) -> NativeCommunity:
    ...
    def load(self, reqmatch=None, ctrl=None) -> NativeCommunity:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.NativeCommunity().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[NativeCommunity]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.NativeCommunity().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
