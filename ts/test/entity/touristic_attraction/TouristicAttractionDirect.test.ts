...
describe('TouristicAttractionDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new ColombiaPublicSDK({
  ...
  test('direct-load-touristic_attraction', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-touristic_attraction', setup.live)) return
  ...
  test('direct-list-touristic_attraction', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-touristic_attraction', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['COLOMBIA_PUBLIC_TEST_TOURISTIC_ATTRACTION_ENTID']
