...
describe('RadioEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'radio.' + op, live)) return
    }
    ...
    const radio_ref01_list = await radio_ref01_ent.list(radio_ref01_match)


    // LOAD
    const radio_ref01_match_dt0: any = {}
    radio_ref01_match_dt0.id = radio_ref01_data.id
    const radio_ref01_data_dt0 = await radio_ref01_ent.load(radio_ref01_match_dt0)
