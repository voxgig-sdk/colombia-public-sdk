...
describe('RegionEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_REGION_ENTID JSON to run live')
      return
    }
    ...
    const region_ref01_data_dt0 = await region_ref01_ent.load(region_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['COLOMBIA_PUBLIC_TEST_REGION_ENTID']
