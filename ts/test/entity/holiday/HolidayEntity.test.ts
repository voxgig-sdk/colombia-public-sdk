...
describe('HolidayEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'holiday.' + op, live)) return
    }
    ...
    const holiday_ref01_list = await holiday_ref01_ent.list(holiday_ref01_match)


    // LOAD
    const holiday_ref01_match_dt0: any = {}
    holiday_ref01_match_dt0.id = holiday_ref01_data.id
    const holiday_ref01_data_dt0 = await holiday_ref01_ent.load(holiday_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['COLOMBIA_PUBLIC_TEST_HOLIDAY_ENTID']
