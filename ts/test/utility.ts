...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.COLOMBIAPUBLIC_TEST_LIVE ||
    'TRUE' === process.env.COLOMBIAPUBLIC_TEST_OVERRIDE
  ) {
  ...
  m.COLOMBIAPUBLIC_TEST_EXPLAIN = process.env.COLOMBIAPUBLIC_TEST_EXPLAIN || m.COLOMBIAPUBLIC_TEST_EXPLAIN
