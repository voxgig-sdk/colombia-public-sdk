...
class ConstitutionArticleEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ColombiaPublicSDK::test(null, null);
        $ent = $testsdk->ConstitutionArticle(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->ConstitutionArticle(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->ConstitutionArticle(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = constitution_article_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "constitution_article." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_CONSTITUTION_ARTICLE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $constitution_article_ref01_data = null;
        if (count($constitution_article_ref01_data_raw) > 0) {
        ...
        $constitution_article_ref01_data_dt0_load_result = Helpers::to_map(is_object($constitution_article_ref01_data_dt0_loaded) && method_exists($constitution_article_ref01_data_dt0_loaded, 'data_get') ? $constitution_article_ref01_data_dt0_loaded->data_get() : $constitution_article_ref01_data_dt0_loaded);
...
function constitution_article_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/constitution_article/ConstitutionArticleTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["constitution_article01", "constitution_article02", "constitution_article03"] as $k) {
    ...
    $entid_env_raw = getenv("COLOMBIA_PUBLIC_TEST_CONSTITUTION_ARTICLE_ENTID");
