...
class TouristicAttractionEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ColombiaPublicSDK::test(null, null);
        $ent = $testsdk->TouristicAttraction(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->TouristicAttraction(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->TouristicAttraction(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = touristic_attraction_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "touristic_attraction." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $touristic_attraction_ref01_data = null;
        if (count($touristic_attraction_ref01_data_raw) > 0) {
...
function touristic_attraction_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/touristic_attraction/TouristicAttractionTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["touristic_attraction01", "touristic_attraction02", "touristic_attraction03"] as $k) {
