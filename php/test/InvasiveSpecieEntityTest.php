...
class InvasiveSpecieEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->InvasiveSpecie(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->InvasiveSpecie(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_INVASIVE_SPECIE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $invasive_specie_ref01_data_dt0_load_result = Helpers::to_map(is_object($invasive_specie_ref01_data_dt0_loaded) && method_exists($invasive_specie_ref01_data_dt0_loaded, 'data_get') ? $invasive_specie_ref01_data_dt0_loaded->data_get() : $invasive_specie_ref01_data_dt0_loaded);
...
function invasive_specie_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("COLOMBIA_PUBLIC_TEST_INVASIVE_SPECIE_ENTID");
