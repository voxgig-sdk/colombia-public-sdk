...
class PresidentEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->President(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->President(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_PRESIDENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $president_ref01_data_dt0_load_result = Helpers::to_map(is_object($president_ref01_data_dt0_loaded) && method_exists($president_ref01_data_dt0_loaded, 'data_get') ? $president_ref01_data_dt0_loaded->data_get() : $president_ref01_data_dt0_loaded);
...
function president_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("COLOMBIA_PUBLIC_TEST_PRESIDENT_ENTID");
