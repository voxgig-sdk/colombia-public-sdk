...
// ColombiaPublic SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ColombiaPublicTestFeature extends ColombiaPublicBaseFeature
{
    ...
    public function init(ColombiaPublicContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ColombiaPublicContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ColombiaPublicParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ColombiaPublicContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ColombiaPublicContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ColombiaPublicParam::call($ctx, $k);
