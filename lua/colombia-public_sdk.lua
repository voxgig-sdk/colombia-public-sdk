...
-- by passing it.
function ColombiaPublicSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ColombiaPublicSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ColombiaPublicSDK: graphql: " .. msg
    res.graphql = errors
  end
