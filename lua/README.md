...
The Lua SDK for the ColombiaPublic API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Airport(nil):list(nil, nil)
...
```lua
local result, err = client:Airport(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:ColombiaPublic(nil):load(
  { id = "test01" }, nil
)
