...
local InvasiveSpecieEntity = {}
InvasiveSpecieEntity.__index = InvasiveSpecieEntity


function InvasiveSpecieEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InvasiveSpecieEntity)
  self._name = "invasive_specie"
  self._client = client
...
function InvasiveSpecieEntity:get_name()
  return self._name
end


function InvasiveSpecieEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InvasiveSpecieEntity.new(self._client, opts)
end
...
function InvasiveSpecieEntity:mark_deleted()
  self._deleted = true
end


function InvasiveSpecieEntity:deleted()
  return true == self._deleted
end


function InvasiveSpecieEntity:data_set(args)
  if args ~= nil then
...
function InvasiveSpecieEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InvasiveSpecieEntity:match_set(args)
  if args ~= nil then
...
function InvasiveSpecieEntity:match_get()
...
function InvasiveSpecieEntity:stream(action, args, callopts)
...
function InvasiveSpecieEntity:_run_op(ctx, post_done)
  local utility = self._utility
