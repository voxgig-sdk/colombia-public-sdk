-- ColombiaPublic SDK Department entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DepartmentEntity = {}
DepartmentEntity.__index = DepartmentEntity


function DepartmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DepartmentEntity)
  self._name = "department"
  self._client = client
...
function DepartmentEntity:get_name()
  return self._name
end


function DepartmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DepartmentEntity.new(self._client, opts)
end
...
function DepartmentEntity:mark_deleted()
  self._deleted = true
end


function DepartmentEntity:deleted()
  return true == self._deleted
end


function DepartmentEntity:data_set(args)
  if args ~= nil then
...
function DepartmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DepartmentEntity:match_set(args)
  if args ~= nil then
...
function DepartmentEntity:match_get()
...
function DepartmentEntity:stream(action, args, callopts)
...
function DepartmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
