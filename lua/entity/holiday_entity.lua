...
local HolidayEntity = {}
HolidayEntity.__index = HolidayEntity


function HolidayEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HolidayEntity)
  self._name = "holiday"
  self._client = client
...
function HolidayEntity:get_name()
  return self._name
end


function HolidayEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HolidayEntity.new(self._client, opts)
end
...
function HolidayEntity:mark_deleted()
  self._deleted = true
end


function HolidayEntity:deleted()
  return true == self._deleted
end


function HolidayEntity:data_set(args)
  if args ~= nil then
...
function HolidayEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HolidayEntity:match_set(args)
  if args ~= nil then
...
function HolidayEntity:match_get()
...
function HolidayEntity:stream(action, args, callopts)
...
function HolidayEntity:_run_op(ctx, post_done)
  local utility = self._utility
