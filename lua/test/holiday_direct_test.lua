...
describe("HolidayDirect", function()
  it("should direct-list-holiday", function()
    local setup = holiday_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-holiday", setup.live and "live" or "unit")
  ...
  it("should direct-load-holiday", function()
    local setup = holiday_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-holiday", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function holiday_direct_setup(mockres)
