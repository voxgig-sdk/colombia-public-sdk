...
describe("NativeCommunityDirect", function()
  it("should direct-list-native_community", function()
    local setup = native_community_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-native_community", setup.live and "live" or "unit")
  ...
  it("should direct-load-native_community", function()
    local setup = native_community_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-native_community", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function native_community_direct_setup(mockres)
