...
describe("RadioDirect", function()
  it("should direct-list-radio", function()
    local setup = radio_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-radio", setup.live and "live" or "unit")
  ...
  it("should direct-load-radio", function()
    local setup = radio_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-radio", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function radio_direct_setup(mockres)
