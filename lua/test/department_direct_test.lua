...
describe("DepartmentDirect", function()
  it("should direct-list-department", function()
    local setup = department_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-department", setup.live and "live" or "unit")
  ...
  it("should direct-load-department", function()
    local setup = department_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-department", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function department_direct_setup(mockres)
