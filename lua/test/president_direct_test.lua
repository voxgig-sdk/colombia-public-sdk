...
describe("PresidentDirect", function()
  it("should direct-list-president", function()
    local setup = president_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-president", setup.live and "live" or "unit")
  ...
  it("should direct-load-president", function()
    local setup = president_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-president", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function president_direct_setup(mockres)
