...
describe("InvasiveSpecieDirect", function()
  it("should direct-list-invasive_specie", function()
    local setup = invasive_specie_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-invasive_specie", setup.live and "live" or "unit")
  ...
  it("should direct-load-invasive_specie", function()
    local setup = invasive_specie_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-invasive_specie", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function invasive_specie_direct_setup(mockres)
