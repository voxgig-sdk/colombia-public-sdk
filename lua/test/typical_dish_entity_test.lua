...
describe("TypicalDishEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:TypicalDish(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:TypicalDish(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_TYPICAL_DISH_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local typical_dish_ref01_data_dt0_load_result = helpers.to_map(type(typical_dish_ref01_data_dt0_loaded) == 'table' and typical_dish_ref01_data_dt0_loaded.data_get and typical_dish_ref01_data_dt0_loaded:data_get() or typical_dish_ref01_data_dt0_loaded)
...
function typical_dish_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("COLOMBIA_PUBLIC_TEST_TYPICAL_DISH_ENTID")
