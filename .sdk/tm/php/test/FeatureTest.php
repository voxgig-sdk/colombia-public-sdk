...
// ColombiaPublic SDK feature test
//
...
class FtCtrl extends ColombiaPublicControl
{
...
class FtClient
{
    ...
    private ColombiaPublicUtility $utility;

    public function __construct(ColombiaPublicUtility $utility, array $options)
    {
    ...
    public function get_utility(): ColombiaPublicUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ColombiaPublicContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ColombiaPublicContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ColombiaPublicContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ColombiaPublicConfig::make_feature($name);
    ...
    private static function buildUrl(ColombiaPublicSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ColombiaPublicError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ColombiaPublicResponse($fetched);
        }
    ...
    private function fail(ColombiaPublicContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ColombiaPublicContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
