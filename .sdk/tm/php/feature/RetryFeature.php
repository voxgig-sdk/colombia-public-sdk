...
// ColombiaPublic SDK retry feature
...
class ColombiaPublicRetryFeature extends ColombiaPublicBaseFeature
{
    ...
    public function init(ColombiaPublicContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ColombiaPublicContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ColombiaPublicContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ColombiaPublicContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
