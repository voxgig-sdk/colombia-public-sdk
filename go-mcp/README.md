...
[MCP](https://modelcontextprotocol.io) server exposing the ColombiaPublic SDK as
two agent tools — `colombia-public_list` and `colombia-public_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/colombia-public-mcp)
make build

# 2. Provide credentials via the environment
export COLOMBIA_PUBLIC_APIKEY=sk_live_xxx
...
```jsonc
// colombia-public_list: first page of records
{ "entity": "airport" }
{ "entity": "airport", "query": { } }

// colombia-public_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/colombia-public-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user colombia-public \
     -- "$PWD"/dist/*/colombia-public-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `colombia-public_list` and `colombia-public_load` tools now appear
   in new sessions. Ask the agent to *"list airport using colombia-public"*
   and it calls `colombia-public_list` with `{"entity":"airport"}`.
...
```sh
export COLOMBIA_PUBLIC_APIKEY=sk_live_xxx            # API key
export COLOMBIA_PUBLIC_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `colombia-public_list` tool
...
### Call the `colombia-public_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `colombia-public_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `colombia-public_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COLOMBIA_PUBLIC_APIKEY` | API key sent with every request. |
| `COLOMBIA_PUBLIC_BASE` | Optional override of the API base URL. |
