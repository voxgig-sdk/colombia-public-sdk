...
module ColombiaPublicUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ColombiaPublicResponse.new({})
    ...
    if fetch_err
      ...
      response = ColombiaPublicResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ColombiaPublicResponse.new(fetched)
    else
