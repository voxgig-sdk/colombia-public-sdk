...
class NaturalAreaEntity
  ...
  def make
    opts = @_entopts.dup
    NaturalAreaEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.NaturalArea.load works with no args).
  ...
  #
  # @param reqmatch [NaturalAreaListMatch, Hash, nil] match filter (any subset of
  #   NaturalArea fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
