...
class TypicalDishEntity
  ...
  def make
    opts = @_entopts.dup
    TypicalDishEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.TypicalDish.load works with no args).
  ...
  #
  # @param reqmatch [TypicalDishListMatch, Hash, nil] match filter (any subset of
  #   TypicalDish fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
