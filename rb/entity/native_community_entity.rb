...
class NativeCommunityEntity
  ...
  def make
    opts = @_entopts.dup
    NativeCommunityEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.NativeCommunity.load works with no args).
  ...
  #
  # @param reqmatch [NativeCommunityListMatch, Hash, nil] match filter (any subset of
  #   NativeCommunity fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
