...
class HolidayEntity
  ...
  def make
    opts = @_entopts.dup
    HolidayEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Holiday.load works with no args).
  ...
  #
  # @param reqmatch [HolidayListMatch, Hash, nil] match filter (any subset of
  #   Holiday fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
