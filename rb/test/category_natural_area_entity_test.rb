...
class CategoryNaturalAreaEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.CategoryNaturalArea(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.CategoryNaturalArea(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = category_natural_area_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "category_natural_area." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_CATEGORY_NATURAL_AREA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    category_natural_area_ref01_data = nil
    if category_natural_area_ref01_data_raw.length > 0
...
def category_natural_area_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "category_natural_area", "CategoryNaturalAreaTestData.json")
  ...
  entid_env_raw = ENV["COLOMBIA_PUBLIC_TEST_CATEGORY_NATURAL_AREA_ENTID"]
