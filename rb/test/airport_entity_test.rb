...
class AirportEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_AIRPORT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    airport_ref01_data_dt0_load_result = Helpers.to_map(airport_ref01_data_dt0_loaded.respond_to?(:data_get) ? airport_ref01_data_dt0_loaded.data_get : airport_ref01_data_dt0_loaded)
...
def airport_basic_setup(extra)
  ...
  entid_env_raw = ENV["COLOMBIA_PUBLIC_TEST_AIRPORT_ENTID"]
