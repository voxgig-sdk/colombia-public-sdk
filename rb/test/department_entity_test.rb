...
class DepartmentEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Department(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Department(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_DEPARTMENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    department_ref01_data_dt0_load_result = Helpers.to_map(department_ref01_data_dt0_loaded.respond_to?(:data_get) ? department_ref01_data_dt0_loaded.data_get : department_ref01_data_dt0_loaded)
...
def department_basic_setup(extra)
  ...
  entid_env_raw = ENV["COLOMBIA_PUBLIC_TEST_DEPARTMENT_ENTID"]
