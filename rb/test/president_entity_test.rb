...
class PresidentEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.President(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.President(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = president_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "president." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_PRESIDENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    president_ref01_data = nil
    if president_ref01_data_raw.length > 0
    ...
    president_ref01_data_dt0_load_result = Helpers.to_map(president_ref01_data_dt0_loaded.respond_to?(:data_get) ? president_ref01_data_dt0_loaded.data_get : president_ref01_data_dt0_loaded)
...
def president_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "president", "PresidentTestData.json")
  ...
  entid_env_raw = ENV["COLOMBIA_PUBLIC_TEST_PRESIDENT_ENTID"]
