// Command ColombiaPublic-cli is a CLI + REPL for the ColombiaPublic SDK,
...
const prompt = "colombia-public"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "airport category_natural_area constitution_article country department holiday invasive_specie map native_community natural_area president radio region touristic_attraction typical_dish"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      COLOMBIA_PUBLIC_APIKEY (api key), COLOMBIA_PUBLIC_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: COLOMBIA_PUBLIC_APIKEY carries the API key and
	// COLOMBIA_PUBLIC_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("COLOMBIA_PUBLIC_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("COLOMBIA_PUBLIC_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewColombiaPublicSDK(opts)
