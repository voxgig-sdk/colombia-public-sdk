...
boru-driven command-line client **and** interactive REPL for the ColombiaPublic
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/colombia-public-cli)
make build

# 2. See usage (words, entities, env vars)
./colombia-public-cli --help
...
./colombia-public-cli load 1 airport            # {id:1} shorthand
./colombia-public-cli load '{id:1}' airport       # explicit match map
./colombia-public-cli list category_natural_area

# 5. Override the API base URL for a single call
COLOMBIA_PUBLIC_BASE=https://api.example.com ./colombia-public-cli list airport

# 6. No arguments -> interactive REPL
./colombia-public-cli
colombia-public> list airport
colombia-public> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/colombia-public-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/colombia-public-cli list airport
   ```
...
```sh
./colombia-public-cli load 1 airport          # scalar shorthand for {id:1}
./colombia-public-cli load '{id:1}' airport     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `colombia-public>`). Each line is
evaluated as its own boru expression:

```text
$ ./colombia-public-cli
colombia-public> list airport
colombia-public> /help
colombia-public> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/colombia-public-cli`. |
...
(`./colombia-public-cli <expr>`) and interactively (the REPL), and expressions compose the
