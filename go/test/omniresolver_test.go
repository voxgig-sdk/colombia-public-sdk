...
import (
	...
	omni "github.com/voxgig-sdk/colombia-public-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ColombiaPublicError); is {
				out := map[string]any{
					"name":    "ColombiaPublicError",
...
func omniCtx(arg any, client *sdk.ColombiaPublicSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ColombiaPublicSDK); is {
			client = live
