...
func TestPresidentEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.President(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.President(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		presidentRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.president")))
		...
		presidentRef01DataDt0LoadResult := core.ToMapAny(entityData(presidentRef01DataDt0Loaded))
...
func presidentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COLOMBIA_PUBLIC_TEST_PRESIDENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COLOMBIA_PUBLIC_TEST_PRESIDENT_ENTID"])
