...
func TestDepartmentEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Department(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Department(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		departmentRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.department")))
		...
		// LOAD
		departmentRef01MatchDt0 := map[string]any{
			"id": departmentRef01Data["id"],
		}
		departmentRef01DataDt0Loaded, err := departmentRef01Ent.Load(departmentRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		departmentRef01DataDt0LoadResult := core.ToMapAny(entityData(departmentRef01DataDt0Loaded))
		if departmentRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if departmentRef01DataDt0LoadResult["id"] != departmentRef01Data["id"] {
...
func departmentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COLOMBIA_PUBLIC_TEST_DEPARTMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COLOMBIA_PUBLIC_TEST_DEPARTMENT_ENTID"])
