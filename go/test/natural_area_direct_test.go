...
func TestNaturalAreaDirect(t *testing.T) {
	t.Run("direct-list-natural_area", func(t *testing.T) {
		setup := natural_areaDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-natural_area", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-natural_area", func(t *testing.T) {
		setup := natural_areaDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-natural_area", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type natural_areaDirectSetupResult struct {
...
func natural_areaDirectSetup(mockres any) *natural_areaDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"COLOMBIA_PUBLIC_TEST_NATURAL_AREA_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["COLOMBIA_PUBLIC_TEST_NATURAL_AREA_ENTID"]; ok {
		...
		return &natural_areaDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &natural_areaDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
