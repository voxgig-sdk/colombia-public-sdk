...
func TestHolidayDirect(t *testing.T) {
	t.Run("direct-list-holiday", func(t *testing.T) {
		setup := holidayDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-holiday", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-holiday", func(t *testing.T) {
		setup := holidayDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-holiday", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type holidayDirectSetupResult struct {
...
func holidayDirectSetup(mockres any) *holidayDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"COLOMBIA_PUBLIC_TEST_HOLIDAY_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["COLOMBIA_PUBLIC_TEST_HOLIDAY_ENTID"]; ok {
		...
		return &holidayDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &holidayDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
