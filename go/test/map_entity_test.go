...
func TestMapEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set COLOMBIAPUBLIC_TEST_MAP_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func mapBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COLOMBIAPUBLIC_TEST_MAP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COLOMBIAPUBLIC_TEST_MAP_ENTID"])
