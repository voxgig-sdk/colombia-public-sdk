...
func TestInvasiveSpecieEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.InvasiveSpecie(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.InvasiveSpecie(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set COLOMBIA_PUBLIC_TEST_INVASIVE_SPECIE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		invasiveSpecieRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.invasive_specie")))
		...
		invasiveSpecieRef01DataDt0LoadResult := core.ToMapAny(entityData(invasiveSpecieRef01DataDt0Loaded))
...
func invasive_specieBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COLOMBIA_PUBLIC_TEST_INVASIVE_SPECIE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COLOMBIA_PUBLIC_TEST_INVASIVE_SPECIE_ENTID"])
