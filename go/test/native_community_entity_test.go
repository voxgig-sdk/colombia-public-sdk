...
func TestNativeCommunityEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.NativeCommunity(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.NativeCommunity(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		nativeCommunityRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.native_community")))
		...
		nativeCommunityRef01DataDt0LoadResult := core.ToMapAny(entityData(nativeCommunityRef01DataDt0Loaded))
...
func native_communityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COLOMBIA_PUBLIC_TEST_NATIVE_COMMUNITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COLOMBIA_PUBLIC_TEST_NATIVE_COMMUNITY_ENTID"])
