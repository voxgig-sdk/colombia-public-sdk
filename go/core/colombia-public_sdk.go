...
func (sdk *ColombiaPublicSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *ColombiaPublicSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("ColombiaPublicSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *ColombiaPublicSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *ColombiaPublicSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("ColombiaPublicSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
